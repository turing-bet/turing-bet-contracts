...
contract DeceptionDerby is
    DeceptionDerbyEvents,
    Centralized,
    PlatformFeeManager
{
    function setPlatformFee(uint feePercent) public override onlyOwner {
        super.setPlatformFee(feePercent);
    }

    constructor(uint _fee) Centralized(msg.sender) PlatformFeeManager(_fee) {}
    ...
    function createPublicLobby(uint ante, uint roomLimit) external {
        lobbyIds++;
    ...
    function joinGame(uint lobbyId) external payable {
    ...
    function startGame(uint lobbyId) external {
        address[] memory participants = participantsOf[lobbyId];
        require(participants[0] == msg.sender, "!initialParticipant");
        require(participants.length > 1, "<2 players");
        require(!locked[lobbyId], "in progress");
        locked[lobbyId] = true;
